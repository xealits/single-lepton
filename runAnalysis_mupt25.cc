  ...
  for (size_t k = 0; k < controlCats.size (); ++k)
    {
      ...
      // delta-R histograms
      mon.addHistogram( new TH1D( icat+"leadleptondeltar",     ";Delta R [R];Events",            100, 0.,  25.  ));
      mon.addHistogram( new TH1D( icat+"leadtaudeltar",        ";Delta R [R];Events",            100, 0.,  25.  ));
      //mon.addHistogram( new TH1D( icat+"taudeltar",            ";Delta R [R];Events",            100, 0.,  25.  ));
    ...
    for (ev.toBegin(); !ev.atEnd(); ++ev)
      {
        ...
        for(size_t ilep=0; ilep<leptons.size (); ++ilep)
          ...
          if(lid == 11 && lepton.pt() < 30.)                      passKin = false;
      ...
      if(isSingleMu || isSingleE){
        ...
        double minDRleadtj(9999.), minDRleadlj(9999.);
        for (size_t ijet = 0; ijet < selJets.size(); ++ijet)
          {
            pat::Jet jet = selJets[ijet];

            //for (size_t ilep = 0; ilep < selLeptons.size(); ilep++)
            // min delta R from the leading lepton to jets
            minDRleadlj = TMath::Min(minDRleadlj, reco::deltaR (jet, selLeptons[0]));
            // min delta R from the leading tau to jets
            minDRleadtj = TMath::Min(minDRleadtj, reco::deltaR (jet, selTaus[0]));
        ...
        for(size_t k=0; k<ctrlCats.size(); ++k){
          ...
          mon.fillHisto (icat+"leadleptondeltar",     tags, minDRleadlj,             weight);
          mon.fillHisto (icat+"leadtaudeltar",        tags, minDRleadtj,             weight);
          //mon.fillHisto (icat+"taudeltar",            tags, minDRtj,             weight);
